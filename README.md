...
**Describe your app in plain English. Webdata Pro builds the database, the admin panel, and the member portal. You design the templates.**

**[⬇ Download ZIP](https://github.com/rytoplay/webdata-pro/archive/refs/heads/main.zip)** &nbsp;·&nbsp; [View on GitHub](https://github.com/rytoplay/webdata-pro)
...
Open **http://localhost:3000/admin** — username `admin`, password `changeme`.

---

## What it does

- **AI app builder** — describe your project in plain English; the AI creates tables, fields, and starter templates in under a minute
- **Visual template editor** — write plain HTML with simple `${table.field}` tokens; live preview before saving
- **Member portal** — login system, groups with access control, per-group home views and custom templates
- **Embeddable views** — drop a `<script>` tag into any website; the widget inherits your existing CSS
- **Public forms** — let anonymous visitors submit data to your database; optional Google reCAPTCHA
- **Email notifications** — alert admins on record changes; immediate or daily digest
- **Two database modes** — SQLite (zero config, single file) or MySQL (multi-user production)
- **No build step** — `npm run dev` and you're in; TypeScript compiles on demand
...
- [Template token reference](docs/templates.md) — All `${field}` tokens, `$if()`, `$order[]`, `$distance[]`, aggregates, public forms, and more
- [Architecture overview](docs/architecture.md) — How the two-database model, embed system, and member portal fit together
- [E-commerce integration](docs/ecommerce.md) — Adding Snipcart or Stripe Checkout to a product catalog
- [Security](docs/security.md) — Permissions model, passwords, 2FA, XSS/injection protection
- [Deployment](docs/deployment.md) — VPS setup, reverse proxy, backups, PM2, cloud hosting
